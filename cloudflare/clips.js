...
export const createClip = async (videoId, startTime, endTime, videoName, watermarkUID) => {
    ...
    try {
        const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/stream/clip`, options);
...
export const getVideoData=async(videoUID)=>{
    ...
    const options = {
     ...
     try {
        const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/stream/${videoUID}`,options);
        const data = await response.json();
        if (!response.ok) {
            throw new Error(`Error: ${data.errors[0].message}`);
        }
...
export const createDownload=async(videoUID)=>{
    ...
    try {
        const response = await fetch(`https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/stream/${videoUID}/downloads`,options);
